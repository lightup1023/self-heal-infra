...
- **서버 식별 정보 (2026-09-15 확인):** 호스트명 `AIOps-Train`. GPU 2장 모두 **RTX 4090 24GB**
  (`nvidia-smi` 기준 24564MiB, 드라이버 595.84, CUDA 13.2), 확인 시점 기준 두 장 다 거의 유휴
  (사용률 0%, 메모리 39MiB/2MiB만 사용 — Ollama가 상시 모델을 올려두진 않는 듯).
- **네트워크 경로: 물리 이전 예정으로 해소 진행 중.** 원래 다른 랙에 있었으나, 사용자가 이 서버를
  OpenStack-IDC와 **같은 랙으로 물리 이전**할 계획. 단, 같은 랙 ≠ 같은 VLAN/L3 경로 자동 보장이므로,
  이전 완료 후 실제로 OpenStack-IDC(또는 그 안의 Nova VM)에서 `AIOps-Train`의 Ollama API(기본 포트
  11434)로 붙는지는 **이전 후 별도로 재확인 필요** (§7에 반영).
...
- **팀 공용 GPU 서버(G1, `AIOps-Train`) 네트워크 경로 — 물리 이전 진행 중**: OpenStack-IDC와 같은 랙으로
  이전 예정(2026-09-15 기준). 이전 완료 후 OpenStack-IDC ↔ `AIOps-Train` 간 Ollama API(11434) 실제 연결
  테스트 필요 — 같은 랙이어도 VLAN/방화벽 분리 여부는 별개 문제라 확인 전까지 미해결로 유지 (§2.3).
